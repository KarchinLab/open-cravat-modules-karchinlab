...
def get_live_annotation_post(request):
    ...
    return get_live_annotation(queries)
...
def get_live_annotation(queries):
    ...
    cedar_config = confloader.get_module_conf('variantreportcedar')
    if 'oc_api_url' not in cedar_config:
        raise abort(500, description='"oc_api_url" not found in variantreportcedar configuration.')
    url = cedar_config['oc_api_url']
    annotators = cedar_config['live_modules']
    all_annotators = []
